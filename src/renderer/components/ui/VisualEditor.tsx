...
export const VisualEditor: React.FC<VisualEditorProps> = ({ onSelectWallpaper }) => {
  const { settings, updateSettings, updateWallSettings, pendingWallpaper, clearPendingWallpaper } = useSceneStore();
...
export const LightContextMenu: React.FC<{
  ...
  onClose: () => void;
  onDelete?: () => void;
}> = ({ x, y, lightId, brightness, temperature, fixtureStyle = 'linear', fixtureLength = 3, isAutoGenerated = false, color, onClose, onDelete }) => {
  ...
  const handleStyleChange = (newStyle: LightFixtureStyle) => {
    ...
    // Clamp length to new style's range
    ...
    >
      ...
      <div className="light-menu-control">
        ...
        <input
          ...
          step="0.1"
          value={fixtureLength}
          onChange={(e) => updateLight(lightId, { fixtureLength: parseFloat(e.target.value) })}
        />
      </div>
      ...
      <div className="light-menu-control">
        ...
        <input
          ...
          step="0.25"
          value={brightness}
          onChange={(e) => updateLight(lightId, { brightness: parseFloat(e.target.value) })}
        />
      </div>
      ...
      <div className="light-menu-control">
        ...
        <input
          ...
          step="100"
          value={temperature}
          onChange={(e) => updateLight(lightId, { temperature: parseFloat(e.target.value) })}
          disabled={!!color}
        />
        <div
        ...
        <div className="color-presets-grid">
          {LIGHT_COLOR_PRESETS.map((preset) => (
            <button
              ...
              onClick={() => updateLight(lightId, { color: preset })}
              title={preset}
            />
          ))}
        </div>
        <div className="color-custom-row">
          ...
          {color && (
            <button
              ...
              onClick={() => updateLight(lightId, { color: undefined })}
            >
